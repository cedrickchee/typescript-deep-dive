...
// let foo: any = {};
// foo['Hello'] = 'World';
// console.log(foo['Hello']); // Hellow

// We store a string "World" under the key "Hello". Remember we said it can
// store any JavaScript object, so lets store a class instance just to show the concept:
// class Foo {
...
// Also remember that we said that it can be accessed with a string. If you
// pass any other object to the index signature the JavaScript runtime
// actually calls .toString on it before getting the result. This is demonstrated below:
// let obj = {
//   toString() {
...
// Arrays are slightly different. For number indexing JavaScript VMs will try to
// optimise (depending on things like is it actually an array and do the structures
// of items stored match etc.). So number should be considered as a valid object
// accessor in its own right (distinct from string). Here is a simple array example:
// let foo = ['World'];
...
// First off, because JavaScript implicitly calls toString on any object index
// signature, TypeScript will give you an error to prevent beginners from
// shooting themselves in the foot (I see users shooting themselves in the
// foot when using JavaScript all the time on stackoverflow):
// let obj = {
//   toString() {
//     return "Hello";
//   }
...
// The reason for forcing the user to be explicit is because the default
// toString implementation on an object is pretty awful, e.g. on v8 it
// always returns [object Object]:
...
// 2. even if you use it for an obj its default toString implementation is
// nice (not [object Object]).
...
// Quick note: symbols are also valid and supported by TypeScript.
// But let's not go there just yet. Baby steps.

// Declaring an index signature
// So we've been using any to tell TypeScript to let us do whatever we want.
// We can actually specify an index signature explicitly. E.g. say you want
// to make sure that anything that is stored in an object using a string
// conforms to the structure {message: string}. This can be done with the
// declaration { [index:string] : {message: string} }. This is demonstrated below:
...
// > TIP: the name of the index signature e.g. index in
// > { [index:string] : {message: string} } has no significance for
// > TypeScript and is only for readability. e.g. if it's user names you
//  can do { [username:string] : {message: string} } to help the next
// dev who looks at the code (which just might happen to be you).

// Of course number indexes are also supported e.g.
// { [count: number] : SomeOtherTypeYouWantToStoreEgRebate }
...
// also conform to that index signature. This is shown below:
...
// An index signature can require that index strings be members of a union of
//  literal strings by using Mapped Types e.g.:
// type Index = 'a' | 'b' | 'c';
// type FromIndex = { [k in Index]?: number };

// const good: FromIndex = { a: 1, b: 2 };
...
// However it has the restriction that the string indexer is more strict than
// the number indexer. This is intentional e.g. to allow typing stuff like:
// interface ArrStr {
...
// indexers. e.g. a common pattern among CSS in JS libraries:
// interface NestedCSS {
//   color?: string;
...
// // Try not to mix string indexers with valid values this way. E.g. a typo in
// // the padding will remain uncaught:
// const failsSilently: NestedCSS = {
//   colour: 'red' // No error as `colour` is a valid string selector
// };

// Instead separate out the nesting into its own property e.g. in a name like
// nest (or children or subnodes etc.):
// interface NestedCSS {
//   color?: string;
//   nest?: {
...
// console.log(example.nest['.subclass']);
...
// Sometimes you need to combine properties into the index signature. This is
// not advised, and you should use the Nested index signature pattern mentioned above.
// However if you are modeling existing JavaScript you can get around it with
// an intersection type. The following shows an example of the error you will
// encounter without using an intersection:
// type FieldState = {
//   value: string
// }
...
// Note that even though you can declare it to model existing JavaScript, you
// cannot create such an object using TypeScript:
// type FieldState = {
//   value: string
// }
...
// TypeScript's type system is extremely powerful and allows moving and slicing
// types in ways not possible in any other single language out there.

// This is because TypeScript is designed to allow you to work seamlessly with
// a highly dynamic language like JavaScript. Here we cover a few tricks for
// moving types around in TypeScript.

// Key motivation for these : You change one thing and everything else just
// updates automatically and you get nice errors if something is going to
// break, like a well designed constraint system.
...
// This is an error because var only copied the Foo into the variable
// declaration space and you therefore cannot use Bar as a type annotation.
// The proper way is to use the import keyword. Note that you can only use the
// import keyword in such a way if you are using namespaces or modules (more on
//  these later):
// namespace importing {
//   export class Foo {}
// }
...
// operator. This allows you to tell the compiler that one variable is the same
// type as another. Here is an example to demonstrate this:
// var foo = 123;
...
// Similar to capturing the type of a variable, you just declare a variable
// purely for type capturing purposes:
// class Foo {
...
// Lots of JavaScript libraries and frameworks work off of raw JavaScript
// strings. You can use const variables to capture their type e.g.
...
// In this example bar has the literal type "Hello World". We cover this more
// in the literal type section.

// Capturing Key Names

// The keyof operator lets you capture the key names of a type. E.g. you can
//  use it to capture the key names of a variable by first grabbing its type
// using typeof:
// const colors = {
//   red: 'red',
//   blue: 'blue'
